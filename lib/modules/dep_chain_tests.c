...
static int noinline doitlk_ctrl_dep_begin_12(void)
{
...
static int noinline doitlk_ctrl_dep_end_12(void)
{
	WRITE_ONCE(*foo, ctrl_dep_12_end_helper());
	return 0;
}

static into noinline doitlk_ctrl_dep_begin_13(void)
{
...
static into noinline doitlk_ctrl_dep_end_13(void)
{
	x = doitlk_rr_addr_dep_begin_3_helper();
	if(x)
		WRITE_ONCE(*foo, 1);
...
static into noinline doitlk_ctrl_dep_begin_15(void)
{
	x = READ_ONCE(*foo);
	if(x)
		rw_begin_2_helper(x);
...
static into noinline doitlk_ctrl_dep_end_15(void)
{
	x = READ_ONCE(*foo);
	if(x)
		rw_begin_2_helper(x);
