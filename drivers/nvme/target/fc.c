...
static inline int nvmet_fc_iodnum(struct nvmet_fc_ls_iod *iodptr)
{
	return (iodptr - iodptr->tgtport->iod);
}

static inline int nvmet_fc_fodnum(struct nvmet_fc_fcp_iod *fodptr)
{
...
static inline u64 nvmet_fc_makeconnid(struct nvmet_fc_tgt_assoc *assoc, u16 qid)
{
	return (assoc->association_id | qid);
}

static inline u64 nvmet_fc_getassociationid(u64 connectionid)
{
	return connectionid & ~NVMET_FC_QUEUEID_MASK;
}

static inline u16 nvmet_fc_getqueueid(u64 connectionid)
{
...
static inline dma_addr_t fc_dma_map_single(struct device *dev, void *ptr,
					   size_t size,
...
static inline int fc_dma_mapping_error(struct device *dev, dma_addr_t dma_addr)
{
	return dev ? dma_mapping_error(dev, dma_addr) : 0;
}

static inline void fc_dma_unmap_single(struct device *dev, dma_addr_t addr,
...
static inline void fc_dma_sync_single_for_cpu(struct device *dev,
...
static inline void fc_dma_sync_single_for_device(struct device *dev,
						 dma_addr_t addr, size_t size,
						 enum dma_data_direction dir)
{
	if (dev)
...
static int fc_map_sg(struct scatterlist *sg, int nents)
{
...
static inline int fc_dma_map_sg(struct device *dev, struct scatterlist *sg,
				int nents, enum dma_data_direction dir)
{
	return dev ? dma_map_sg(dev, sg, nents, dir) : fc_map_sg(sg, nents);
}

static inline void fc_dma_unmap_sg(struct device *dev, struct scatterlist *sg,
				   int nents, enum dma_data_direction dir)
{
	if (dev)
...
static void __nvmet_fc_finish_ls_req(struct nvmet_fc_ls_req_op *lsop)
{
...
static int __nvmet_fc_send_ls_req(struct nvmet_fc_tgtport *tgtport,
				  struct nvmet_fc_ls_req_op *lsop,
				  void (*done)(struct nvmefc_ls_req *req,
					       int status))
{
	struct nvmefc_ls_req *lsreq = &lsop->ls_req;
	unsigned long flags;
...
static int nvmet_fc_send_ls_req_async(struct nvmet_fc_tgtport *tgtport,
				      struct nvmet_fc_ls_req_op *lsop,
				      void (*done)(struct nvmefc_ls_req *req,
						   int status))
{
...
static void nvmet_fc_disconnect_assoc_done(struct nvmefc_ls_req *lsreq,
					   int status)
{
	struct nvmet_fc_ls_req_op *lsop =
 ...
 */
static void nvmet_fc_xmt_disconnect_assoc(struct nvmet_fc_tgt_assoc *assoc)
{
	...
	lsop = kzalloc((sizeof(*lsop) + sizeof(*discon_rqst) +
			sizeof(*discon_acc) + tgtport->ops->lsrqst_priv_sz),
		       GFP_KERNEL);
	if (!lsop) {
...
static int nvmet_fc_alloc_ls_iodlist(struct nvmet_fc_tgtport *tgtport)
{
	struct nvmet_fc_ls_iod *iod;
	int i;
	...
	for (i = 0; i < NVMET_LS_CTX_COUNT; iod++, i++) {
		...
		iod->rqstbuf =
			kzalloc(sizeof(union nvmefc_ls_requests) +
...
static void nvmet_fc_free_ls_iodlist(struct nvmet_fc_tgtport *tgtport)
{
...
static void nvmet_fc_free_ls_iod(struct nvmet_fc_tgtport *tgtport,
				 struct nvmet_fc_ls_iod *iod)
{
	unsigned long flags;
...
static void nvmet_fc_prep_fcp_iodlist(struct nvmet_fc_tgtport *tgtport,
				      struct nvmet_fc_tgt_queue *queue)
{
...
static void nvmet_fc_destroy_fcp_iodlist(struct nvmet_fc_tgtport *tgtport,
					 struct nvmet_fc_tgt_queue *queue)
{
	struct nvmet_fc_fcp_iod *fod = queue->fod;
	int i;
...
static void nvmet_fc_queue_fcp_req(struct nvmet_fc_tgtport *tgtport,
				   struct nvmet_fc_tgt_queue *queue,
				   struct nvmefc_tgt_fcp_req *fcpreq)
{
	 ...
	 */
	fcpreq->hwqid =
		queue->qid ? ((queue->qid - 1) % tgtport->ops->max_hw_queues) :
			     0;
...
static void nvmet_fc_free_fcp_iod(struct nvmet_fc_tgt_queue *queue,
				  struct nvmet_fc_fcp_iod *fod)
{
...
nvmet_fc_alloc_target_queue(struct nvmet_fc_tgt_assoc *assoc, u16 qid,
			    u16 sqsize)
{
	struct nvmet_fc_tgt_queue *queue;
	int ret;
...
static void nvmet_fc_tgt_queue_free(struct kref *ref)
{
	struct nvmet_fc_tgt_queue *queue =
...
static void nvmet_fc_delete_target_queue(struct nvmet_fc_tgt_queue *queue)
{
	...
	for (;;) {
		deferfcp = list_first_entry_or_null(
			&queue->pending_cmd_list, struct nvmet_fc_defer_fcp_req,
			req_list);
		if (!deferfcp)
			break;
...
static struct nvmet_fc_tgt_queue *
nvmet_fc_find_target_queue(struct nvmet_fc_tgtport *tgtport, u64 connection_id)
{
	struct nvmet_fc_tgt_assoc *assoc;
	struct nvmet_fc_tgt_queue *queue;
...
static void nvmet_fc_hostport_free(struct kref *ref)
{
	struct nvmet_fc_hostport *hostport =
...
static void nvmet_fc_hostport_put(struct nvmet_fc_hostport *hostport)
{
	kref_put(&hostport->ref, nvmet_fc_hostport_free);
}

static int nvmet_fc_hostport_get(struct nvmet_fc_hostport *hostport)
{
	return kref_get_unless_zero(&hostport->ref);
}

static void nvmet_fc_free_hostport(struct nvmet_fc_hostport *hostport)
{
...
static void nvmet_fc_delete_assoc(struct work_struct *work)
{
	struct nvmet_fc_tgt_assoc *assoc =
...
static void nvmet_fc_target_assoc_free(struct kref *ref)
{
	struct nvmet_fc_tgt_assoc *assoc =
	...
	dev_info(tgtport->dev, "{%d:%d} Association freed\n",
		 tgtport->fc_target_port.port_num, assoc->a_id);
	kfree_rcu(assoc, rcu);
	nvmet_fc_tgtport_put(tgtport);
}
...
static struct nvmet_fc_tgt_assoc *
nvmet_fc_find_target_assoc(struct nvmet_fc_tgtport *tgtport, u64 association_id)
{
	struct nvmet_fc_tgt_assoc *assoc;
...
static void nvmet_fc_portentry_bind(struct nvmet_fc_tgtport *tgtport,
				    struct nvmet_fc_port_entry *pe,
				    struct nvmet_port *port)
{
...
static void nvmet_fc_portentry_unbind(struct nvmet_fc_port_entry *pe)
{
	unsigned long flags;
 ...
 */
static void nvmet_fc_portentry_unbind_tgt(struct nvmet_fc_tgtport *tgtport)
{
	struct nvmet_fc_port_entry *pe;
	unsigned long flags;
 ...
 */
static void nvmet_fc_portentry_rebind_tgt(struct nvmet_fc_tgtport *tgtport)
{
	struct nvmet_fc_port_entry *pe;
	unsigned long flags;
 ...
 */
int nvmet_fc_register_targetport(struct nvmet_fc_port_info *pinfo,
				 ...
				 struct nvmet_fc_target_port **portptr)
{
	...
	if (!template->xmt_ls_rsp || !template->fcp_op ||
	    !template->fcp_abort || !template->fcp_req_release ||
	    !template->targetport_delete || !template->max_hw_queues ||
	    !template->max_sgl_segments || !template->max_dif_sgl_segments ||
...
static void nvmet_fc_free_tgtport(struct kref *ref)
{
	struct nvmet_fc_tgtport *tgtport =
...
static void __nvmet_fc_free_assocs(struct nvmet_fc_tgtport *tgtport)
{
	struct nvmet_fc_tgt_assoc *assoc;
 ...
 */
void nvmet_fc_invalidate_host(struct nvmet_fc_target_port *target_port,
			      void *hosthandle)
{
	...
	spin_lock_irqsave(&tgtport->lock, flags);
	list_for_each_entry_safe(assoc, next, &tgtport->assoc_list, a_list) {
		if (!assoc->hostport ||
 ...
 */
static void nvmet_fc_delete_ctrl(struct nvmet_ctrl *ctrl)
{
 ...
 * (ex: -ENXIO) upon failure.
 */
int nvmet_fc_unregister_targetport(struct nvmet_fc_target_port *target_port)
{
	struct nvmet_fc_tgtport *tgtport = targetport_to_tgtport(target_port);
...
static void nvmet_fc_ls_create_association(struct nvmet_fc_tgtport *tgtport,
					   struct nvmet_fc_ls_iod *iod)
{
	...
	if (iod->rqstdatalen < FCNVME_LSDESC_CRA_RQST_MINLEN)
		...
		iod->assoc =
			nvmet_fc_alloc_target_assoc(tgtport, iod->hosthandle);
		if (!iod->assoc)
			ret = VERR_ASSOC_ALLOC_FAIL;
		else {
			queue = nvmet_fc_alloc_target_queue(
				iod->assoc, 0,
				be16_to_cpu(rqst->assoc_cmd.sqsize));
			if (!queue) {
	...
	if (ret) {
		...
		iod->lsrsp->rsplen = nvme_fc_format_rjt(acc, sizeof(*acc),
							rqst->w0.ls_cmd,
							FCNVME_RJT_RC_LOGIC,
	...
	nvme_fc_format_rsp_hdr(
		acc, FCNVME_LS_ACC,
		fcnvme_lsdesc_len(sizeof(struct fcnvme_ls_cr_assoc_acc)),
		FCNVME_LS_CREATE_ASSOCIATION);
	acc->associd.desc_tag = cpu_to_be32(FCNVME_LSDESC_ASSOC_ID);
	acc->associd.desc_len =
		fcnvme_lsdesc_len(sizeof(struct fcnvme_lsdesc_assoc_id));
...
static void nvmet_fc_ls_create_connection(struct nvmet_fc_tgtport *tgtport,
					  struct nvmet_fc_ls_iod *iod)
{
	...
	if (iod->rqstdatalen < sizeof(struct fcnvme_ls_cr_conn_rqst))
		ret = VERR_CR_CONN_LEN;
		 ...
		 fcnvme_lsdesc_len(sizeof(struct fcnvme_ls_cr_conn_rqst)))
		...
		ret = VERR_CR_CONN_CMD;
		 ...
		 fcnvme_lsdesc_len(sizeof(struct fcnvme_lsdesc_cr_conn_cmd)))
		...
		/* new io queue */
		iod->assoc = nvmet_fc_find_target_assoc(
			tgtport, be64_to_cpu(rqst->associd.association_id));
		if (!iod->assoc)
			ret = VERR_NO_ASSOC;
		else {
			queue = nvmet_fc_alloc_target_queue(
				iod->assoc, be16_to_cpu(rqst->connect_cmd.qid),
				be16_to_cpu(rqst->connect_cmd.sqsize));
			if (!queue)
				ret = VERR_QUEUE_ALLOC_FAIL;
	...
	if (ret) {
		dev_err(tgtport->dev, "Create Connection LS failed: %s\n",
			validation_errors[ret]);
		iod->lsrsp->rsplen = nvme_fc_format_rjt(
			acc, sizeof(*acc), rqst->w0.ls_cmd,
	...
	acc->connectid.desc_len =
		fcnvme_lsdesc_len(sizeof(struct fcnvme_lsdesc_conn_id));
	acc->connectid.connection_id = cpu_to_be64(nvmet_fc_makeconnid(
		iod->assoc, be16_to_cpu(rqst->connect_cmd.qid)));
}
 ...
 */
static int nvmet_fc_ls_disconnect(struct nvmet_fc_tgtport *tgtport,
				  struct nvmet_fc_ls_iod *iod)
{
	...
	if (!ret) {
		...
		assoc = nvmet_fc_find_target_assoc(
			tgtport, be64_to_cpu(rqst->associd.association_id));
		iod->assoc = assoc;
		if (!assoc)
			ret = VERR_NO_ASSOC;
	...
	if (ret || !assoc) {
		dev_err(tgtport->dev, "Disconnect LS failed: %s\n",
			validation_errors[ret]);
		iod->lsrsp->rsplen = nvme_fc_format_rjt(
			acc, sizeof(*acc), rqst->w0.ls_cmd,
			(ret == VERR_NO_ASSOC) ? FCNVME_RJT_RC_INV_ASSOC :
	...
	if (oldls) {
		...
		oldls->lsrsp->rsplen =
			nvme_fc_format_rjt(oldls->rspbuf, sizeof(*iod->rspbuf),
...
static void nvmet_fc_xmt_ls_rsp_done(struct nvmefc_ls_rsp *lsrsp)
{
 ...
 */
static void nvmet_fc_handle_ls_rqst(struct nvmet_fc_tgtport *tgtport,
				    struct nvmet_fc_ls_iod *iod)
{
	...
	default:
		iod->lsrsp->rsplen = nvme_fc_format_rjt(
			iod->rspbuf, sizeof(*iod->rspbuf), w0->ls_cmd,
 ...
 */
int nvmet_fc_rcv_ls_req(struct nvmet_fc_target_port *target_port,
			void *hosthandle, struct nvmefc_ls_rsp *lsrsp,
...
static int nvmet_fc_alloc_tgt_pgs(struct nvmet_fc_fcp_iod *fod)
{
...
static void nvmet_fc_free_tgt_pgs(struct nvmet_fc_fcp_iod *fod)
{
	if (!fod->data_sg || !fod->data_sg_cnt)
		return;

	fc_dma_unmap_sg(fod->tgtport->dev, fod->data_sg, fod->data_sg_cnt,
			((fod->io_dir == NVMET_FCP_WRITE) ? DMA_FROM_DEVICE :
							    DMA_TO_DEVICE));
...
static bool queue_90percent_full(struct nvmet_fc_tgt_queue *q, u32 sqhd)
{
	u32 sqtail, used;
 ...
 */
static void nvmet_fc_prep_fcp_rsp(struct nvmet_fc_tgtport *tgtport,
				  struct nvmet_fc_fcp_iod *fod)
{
...
static void nvmet_fc_abort_op(struct nvmet_fc_tgtport *tgtport,
			      struct nvmet_fc_fcp_iod *fod)
{
	struct nvmefc_tgt_fcp_req *fcpreq = fod->fcpreq;
...
static void nvmet_fc_xmt_fcp_rsp(struct nvmet_fc_tgtport *tgtport,
				 struct nvmet_fc_fcp_iod *fod)
{
	int ret;
...
static void nvmet_fc_transfer_fcp_data(struct nvmet_fc_tgtport *tgtport,
				       struct nvmet_fc_fcp_iod *fod, u8 op)
{
	...
	fcpreq->sg = sg;
	fcpreq->sg_cnt = 0;
	while (tlen < remaininglen && fcpreq->sg_cnt < tgtport->max_sg_cnt &&
	...
	if ((op == NVMET_FCOP_READDATA) &&
	    ((fod->offset + fcpreq->transfer_length) ==
	     fod->req.transfer_len) &&
...
static inline bool __nvmet_fc_fod_op_abort(struct nvmet_fc_fcp_iod *fod,
					   bool abort)
{
	struct nvmefc_tgt_fcp_req *fcpreq = fod->fcpreq;
	struct nvmet_fc_tgtport *tgtport = fod->tgtport;
 ...
 */
static void nvmet_fc_fod_op_done(struct nvmet_fc_fcp_iod *fod)
{
	struct nvmefc_tgt_fcp_req *fcpreq = fod->fcpreq;
	struct nvmet_fc_tgtport *tgtport = fod->tgtport;
	unsigned long flags;
 ...
 */
static void __nvmet_fc_fcp_nvme_cmd_done(struct nvmet_fc_tgtport *tgtport,
					 struct nvmet_fc_fcp_iod *fod,
					 int status)
{
	struct nvme_common_command *sqe = &fod->cmdiubuf.sqe.common;
				     ...
				     struct nvmet_fc_fcp_iod *fod)
{
	...
	ret = nvmet_req_init(&fod->req, &fod->queue->nvme_cq,
			     &fod->queue->nvme_sq, &nvmet_fc_tgt_fcp_ops);
	if (!ret) {
 ...
 */
int nvmet_fc_rcv_fcp_req(struct nvmet_fc_target_port *target_port,
			 struct nvmefc_tgt_fcp_req *fcpreq, void *cmdiubuf,
			 u32 cmdiubuf_len)
{
	struct nvmet_fc_tgtport *tgtport = targetport_to_tgtport(target_port);
 ...
 */
void nvmet_fc_rcv_fcp_abort(struct nvmet_fc_target_port *target_port,
			    struct nvmefc_tgt_fcp_req *fcpreq)
{
	struct nvmet_fc_fcp_iod *fod = fcpreq->nvmet_fc_private;
	struct nvmet_fc_tgt_queue *queue;
...
static int __nvme_fc_parse_u64(substring_t *sstr, u64 *val)
{
	u64 token64;
 ...
 */
static int nvme_fc_parse_traddr(struct nvmet_fc_traddr *traddr, char *buf,
				size_t blen)
{
	...
	if (strnlen(buf, blen) == NVME_FC_TRADDR_MAXLENGTH &&
	    ...
	    !strncmp(&buf[NVME_FC_TRADDR_MAX_PN_OFFSET], "pn-0x",
		     NVME_FC_TRADDR_OXNNLEN)) {
		nnoffset = NVME_FC_TRADDR_OXNNLEN;
		pnoffset =
			NVME_FC_TRADDR_MAX_PN_OFFSET + NVME_FC_TRADDR_OXNNLEN;
	} else if ((strnlen(buf, blen) == NVME_FC_TRADDR_MINLENGTH &&
		    !strncmp(buf, "nn-", NVME_FC_TRADDR_NNLEN) &&
		    !strncmp(&buf[NVME_FC_TRADDR_MIN_PN_OFFSET], "pn-",
...
static int nvmet_fc_add_port(struct nvmet_port *port)
{
	struct nvmet_fc_tgtport *tgtport;
	struct nvmet_fc_port_entry *pe;
...
static void nvmet_fc_remove_port(struct nvmet_port *port)
{
...
static void nvmet_fc_discovery_chg(struct nvmet_port *port)
{
	struct nvmet_fc_port_entry *pe = port->priv;
